	...
	while (true) {
		...
		switch (type) {
			...
			case 0xff:
				...
				return true;
			default:
				console.log("Unknow ctrl sequence command:" + type);
				return false;
		}
	}
}
...
SPUControl.prototype.ExtractAlpha = function (stream) {
	...
	if (false) {
		...
		//according to http://dvd.sourceforge.net/spu_notes
		//with this correction, the subtitle is good.
		this.Alpha.push((Data & 0xF000));
		this.Alpha.push((Data & 0x0F00) >> 4);
		this.Alpha.push((Data & 0x00F0) >> 8);
		this.Alpha.push((Data & 0x000F) >> 12);
	}
}
